...
func (c *Client) runWithJSON(ctx context.Context, req *Request, resp interface{}) error {
	...
	if err := json.NewEncoder(&requestBody).Encode(requestBodyObj); err != nil {
		return errors.Join(err, fmt.Errorf("encode body"))
	}
	...
	if _, err := io.Copy(&buf, res.Body); err != nil {
		return errors.Join(err, fmt.Errorf("reading body"))
	}
	c.logf("<< %s", buf.String())
	if err := json.NewDecoder(&buf).Decode(&gr); err != nil {
		...
		return errors.Join(err, fmt.Errorf("decoding response"))
	}
	if len(gr.Errors) > 0 {
		// return first error
		return gr.Errors[0]
	...
	if gr.Error != nil {
		return graphErr{Message: *gr.Error}
	}
	return nil
}
...
func (c *Client) runWithPostFields(ctx context.Context, req *Request, resp interface{}) error {
	...
	if err := writer.WriteField("query", req.q); err != nil {
		return errors.Join(err, fmt.Errorf("write query field"))
	}
	var variablesBuf bytes.Buffer
	if len(req.vars) > 0 {
		variablesField, err := writer.CreateFormField("variables")
		if err != nil {
			return errors.Join(err, fmt.Errorf("create variables field"))
		}
		if err := json.NewEncoder(io.MultiWriter(variablesField, &variablesBuf)).Encode(req.vars); err != nil {
			return errors.Join(err, fmt.Errorf("encode variables"))
		}
	}
	for i := range req.files {
		part, err := writer.CreateFormFile(req.files[i].Field, req.files[i].Name)
		if err != nil {
			return errors.Join(err, fmt.Errorf("create form file"))
		}
		if _, err := io.Copy(part, req.files[i].R); err != nil {
			return errors.Join(err, fmt.Errorf("preparing file"))
		}
	}
	if err := writer.Close(); err != nil {
		return errors.Join(err, fmt.Errorf("close writer"))
	}
